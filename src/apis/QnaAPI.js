...
const host = "https://allmartsystem.shop/api/v1/qna";
...
export const fetchQuestionById = async (qno) => {
    try {
        const response = await axios.get(`${host}/question/${qno}`, { withCredentials: true });
        return response.data;
    } catch (error) {
...
export const deleteAnswer = async (ano) => {
    const response = await axios.delete(`${host}/answer/${ano}`,{withCredentials: true});
    return response.data;
};

// 답변 수정
export const updateAnswer = async (ano, answerData) => {
    const response = await axios.put(`${host}/answer/${ano}`, answerData,{withCredentials: true});
    return response.data;
};
